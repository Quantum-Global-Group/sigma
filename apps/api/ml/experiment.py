...
Usage:
    ...
    with start_run("train-ensemble-equity-v1.0", tags={"asset_class": "equity"}) as run:
...
def _load_mlflow() -> Any | None:
    ...
    try:
        ...
        mlflow.set_experiment(settings.mlflow_experiment or "sigma-training")
        _mlflow = mlflow
        logger.info(
            "MLflow tracking enabled (uri=%s, experiment=%s)",
            settings.mlflow_tracking_uri or "local ./mlruns", settings.mlflow_experiment,
        )
