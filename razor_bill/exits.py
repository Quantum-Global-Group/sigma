...
import pandas as pd
from .db import ExitStateRow, SessionLocal
...
async def load_exit_state() -> Dict[str, dict]:
    ...
    async with SessionLocal() as session:
        res = await session.execute(ExitStateRow.__table__.select())  # type: ignore[attr-defined]
        rows = res.fetchall()
        for r in rows:
            s = str(r._mapping["symbol"])  # type: ignore[index]
            took = bool(int(r._mapping["took_partial"]))  # type: ignore[index]
            be = float(r._mapping["breakeven_px"])  # type: ignore[index]
            tt = float(r._mapping["tight_trailing"])  # type: ignore[index]
            state[s] = {"took_partial": took, "breakeven_px": (be if be > 0 else None), "tight_trailing": (tt if tt > 0 else None)}
    return state
    ...
    async with SessionLocal() as session:
        ...
        for sym, st in state.items():
            await session.execute(
                ExitStateRow.__table__.delete().where(ExitStateRow.symbol == sym)  # type: ignore[attr-defined]
            )
            session.add(
                ExitStateRow(
                    symbol=sym,
                    took_partial=int(bool(st.get("took_partial", False))),
                    breakeven_px=float(st.get("breakeven_px", 0.0) or 0.0),
                    tight_trailing=float(st.get("tight_trailing", 0.0) or 0.0),
                )
            )
